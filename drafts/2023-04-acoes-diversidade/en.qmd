...
grafico_colunas <- function(dados, col, y_label){
  ...
  dplyr::count(coluna) |> 
  dplyr::mutate(
    porc_raw = round(n/sum(n), 2),
    porc = scales::percent(porc_raw) ,
    coluna = forcats::fct_reorder(coluna, n)) |>
  ggplot() +
  aes(y = coluna, x = n) +
  geom_col(fill = "#562457") +
  labs(x = "% of responses", y = y_label) +
  geom_label(aes(x = n+8, y = coluna, label = porc)) +
  rladies_theme
}
...
censo_2020 |>
    ...
    genero = dplyr::case_match(
      genero,
      "Mulher (cis ou trans)" ~ "Woman - Cis or Transgender",
      "Homem (cis ou trans)" ~ "Men - Cis or Transgender",
